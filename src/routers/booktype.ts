...
router.get('/',upload.none(),async(req:Request,res:Response)=>{
    try{
        const result = await prisma.bookType.findMany({
            where: {
                deleted: {
                    equals: null,
                },
...
router.post('/', 
    ...
    async (req:Request,res:Response)=>{
        ...
        const { value ,error } = BookType.validate(req.body)
        ...
        try{
            const result = await prisma.bookType.create({
                data:value
            })
...
router.put('/:id',upload.none(),async(req:Request,res:Response)=>{
    try{
        ...
        const result = await prisma.bookType.upsert({
            where: {
                id: parseInt(req.params['id'])
            },
            update: value,
...
router.delete('/:id',upload.none(),async(req:Request,res:Response)=>{
    try{
        ...
        const result = await prisma.bookType.update({
            where: {
                id: parseInt(req.params['id'])
            },
